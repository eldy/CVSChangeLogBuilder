...
if (! $copyalreadydone) {
	...
	mkdir "$BUILDROOT";
	print "Recopie de $SOURCE/cvschangelogbuilder.pl dans $BUILDROOT/$PROJECT\n";
	mkdir "$BUILDROOT/$PROJECT";
	$ret=`cp -p "$SOURCE/cvschangelogbuilder.pl" "$BUILDROOT/$PROJECT"`;
...
$ret=`rm -f $BUILDROOT/$PROJECT/$PROJECT*.athena.*`;
...
#------------------------------
    ...
    if ($CHOOSEDTARGET{$target} < 0) { next; }
    	...
    	if ($target eq 'TGZ') {
    		...
    		$ret=`tar --directory="$BUILDROOT" --group=500 --owner=500 -czvf $FILENAMETGZ.tar.gz $FILENAMETGZ`;
